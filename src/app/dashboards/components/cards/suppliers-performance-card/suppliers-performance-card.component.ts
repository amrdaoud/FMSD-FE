...
export class SuppliersPerformanceCardComponent {
  ...
  reportFillingChart = toSignal(
    toObservable(this.dateFilter).pipe(
      switchMap((p) =>
        this.dashboardService.getSupplierPerformanceCard(p, true)
      )
    ),
    { initialValue: { datasets: [], labels: [], values: [] } }
  );

  reportDistributionChart = toSignal(
    toObservable(this.dateFilter).pipe(
      switchMap((p) =>
        this.dashboardService.getSupplierPerformanceCard(p, false)
      )
    ),
    { initialValue: { datasets: [], labels: [], values: [] } }
  );
