...
export class CalibrationConsts {
  ...
  private static lastWeekDate = new Date(
  ...
  static get filters(): DataTableFilter[] {
    return [
...

      {
        ...
        controlName: 'cities',
        isMulti: true,
        data$: inject(GenericService).getCities(),
        isLoading: inject(GenericService).loadingCities,
        label: 'City',
      },
      {
        type: 'select',
        controlName: 'stationGuids',
        isMulti: true,
        data$: inject(GenericService).getStations(),
        isLoading: inject(GenericService).loadingStations,
        label: 'Station',
        valueProperty: 'guid',
        displayProperty: 'stationName',
      },
      {
        type: 'select',
        controlName: 'tankGuids',
        isMulti: true,
        data$: inject(GenericService).getTanks(),
        isLoading: inject(GenericService).loadingTanks,
        label: 'Tanks',
        valueProperty: 'guid',
        displayProperty: 'tankName',
      },
      {
        type: 'select',
        controlName: 'pumpIds',
        isMulti: true,
        data$: inject(GenericService).getStationPumbs(),
        isLoading: inject(GenericService).loadingTanks,
        label: 'Pumbs Number',
        valueProperty: 'id',
